define(function (require, exports, module) {
    ...
    var global = {
        ...
        parseItemDataToMenu: function (item_data) {
            let datas = {};
            item_data.forEach(function (item) {
                // console.log(item);
                if (!datas.hasOwnProperty(item.type)) {
                    datas[item.type] = {};
                    datas[item.type]['type'] = item.type;
                    datas[item.type]['type_name'] = item.type_name;
                    datas[item.type]['items'] = {};
                    datas[item.type]['group'] = [];
                }
                if (!datas[item.type]['items'].hasOwnProperty(item.group)) {
                    datas[item.type]['items'][item.group] = [];
                }
                if (datas[item.type]['group'].indexOf(item.group) === -1) {
                    datas[item.type]['group'].push(item.group);
                }
                if (!datas[item.type].hasOwnProperty('parameters')) {
                    datas[item.type]['parameters'] = item.parameters.map(function (i) {
                        return i.name;
                    });
                } else {
                    item.parameters.map(function (i) {
                        if (datas[item.type]['parameters'].indexOf(i.name) === -1) {
                            datas[item.type]['parameters'].push(i.name);
                        }
                    });
                }
                let keyed_parameters = {};
                item.parameters.map(function (i) {
                    keyed_parameters[i.name] = i;
                });
                item['keyed_parameters'] = keyed_parameters;
                datas[item.type]['items'][item.group].push(item);
            });

            let menu = [];
            let datas_keys = Object.keys(datas);
            datas_keys.forEach(function (item) {
                let data = datas[item];
                menu.push([
                    data.type_name,
                    data.type,
        ...
        growthRate: function (curr, comp) {
            return comp != 0 ? (curr - comp) / comp : 0;
        },
    }
